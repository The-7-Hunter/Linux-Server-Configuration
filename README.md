...
## IP
- IP Address: http://35.182.78.133.xip.io/ or http://ec2-35-182-78-133.ca-central-1.compute.amazonaws.com/
...
The public IP address of the instance  `35.182.78.133`.


# Linux Configuration
 1. Download your instance Private Key from your profile.
 2. in your local machine, cd to ` ⁨Users⁩ ▸ [your user] ▸ .ssh`.
 3. move your private key file into this directory.
 4. name it whatever you like.
 5. change permission using `chmod 600 ~/.ssh/keyname.pem`.
 6. log into our Amazon Lightsail Server: `$ ssh -i ~/.ssh/keyname.pem ubuntu@[PUBLIC IP ADDRESS]`.
...
3. ` sudo apt-get install finger`. installation of finger is not required, however it makes it easier to manage users in your server.

## generate public key
1. in another terminal write this `ssh-keygen -f ~/.ssh/` it will ask for password that grader will use to access the machine.
2. `cat ~/.ssh/nameOfTheKey.pub` to find the public key of that password. COPY IT.
...
## log in as grader
using `ssh -i ~/.ssh/nameOfTheKey grader@35.182.78.133`
- enable key authentication instead of password authentication and resetting PasswordAuthentication to NO and change port to 2200 using `sudo nano /etc/ssh/sshd_config`.
...
- BEFORE disconnecting and accessing server with port 2200, we need to setup firewall or else we will lose our access to the server:
  ...
  - `sudo ufw enable`
- Diconnect and log in using `ssh -i ~/.ssh/nameOfTheKey.rsa grader@35.182.78.133 -p 2200`
...
## setup Google oAuth properly :
  ...
  - Add authorized Javascript origins: `http://35.182.78.133.xip.io`
  - Add authorised redirect URIs:             `http://35.182.78.133.xip.io/gconnect` and `http://35.182.78.133.xip.io/login`
...
### FINALYY :
`sudo service apache2 restart` and open up your ip address in the browser with extension `xip.io` in my machine I opened http://35.182.78.133.xip.io.
